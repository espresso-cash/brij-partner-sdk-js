...
Create your key pair using the `KycPartnerClient.generateKeyPair()` method. Store the seed returned by this method in a safe place.
...
```Javascript
const client = await KycPartnerClient.fromSeed(seed);
```
